...
export function ImagePlaceholder({ src, alt, label, ratio = "wide", priority = false, className }: ImagePlaceholderProps) {
  return (
    <MotionItem distance={28}>
      <figure className={cn("image-frame isolate will-change-transform", ratios[ratio], className)}>
        <Image src={src} alt={alt} fill sizes="(min-width: 1024px) 50vw, 100vw" priority={priority} className="object-cover" />
        ...
        <figcaption className="absolute bottom-0 left-0 right-0 flex items-center justify-between border-t border-white/15 bg-ink/35 px-5 py-4 text-xs font-bold uppercase tracking-[0.16em] text-paper backdrop-blur-sm">
          <span>{label}</span>
          <span className="h-px w-12 bg-primary transition-all duration-500 group-hover:w-16" />
        </figcaption>
      </figure>
    </MotionItem>
  );
